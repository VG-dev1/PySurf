from PyQt5.QtWidgets import (QApplication, QMainWindow, QLineEdit, QPushButton, QVBoxLayout,
                             ...
                             QToolButton, QFileDialog, QShortcut, QFrame, QScrollArea, QProgressBar, QMessageBox)
...
class SecurityReportDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Security Report")
    ...
    def show_report(self, status, description, color):
...
class HomePage(QWidget):
    def __init__(self, main_window, is_incognito=False, parent=None):
        super().__init__(parent)
        self.main_window = main_window
        ...
        title_text = "PySurf Search"
        if self.is_incognito:
            title_text = "PySurf Incognito Search"
        ...
        def search():
            ...
            if query:
                self.main_window.open_new_tab(self.main_window.current_search_engine['url'] + query)
...
class PySurfWebPage(QWebEnginePage):
...
class BrowserTab(QWidget):
    def __init__(self, parent, url_string, main_window, profile, is_incognito=False):
        ...
        self.web_page = PySurfWebPage(main_window, profile, self.browser)
        ...
        self.browser.urlChanged.connect(lambda qurl: self.main_window._handle_url_change(self, qurl))
...
class IncognitoWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PySurf Incognito")
        self.setWindowIcon(QIcon(os.path.join(ICON_PATH, "Incognito_Icon.svg")))
        self.resize(1200, 700)
        
        self.profile = QWebEngineProfile("incognito_profile_" + str(uuid.uuid4()), self)
        ...
        self.security_icon_button = QToolButton()
        self.security_icon_button.setIconSize(QSize(20, 20))
        self.security_icon_button.clicked.connect(self.show_security_report)
        url_layout.addWidget(self.security_icon_button)
        ...
        plus_button.setIcon(QIcon(os.path.join(ICON_PATH, "Plus_Icon.svg")))
        ...
        self.search_engines = {
            "Google": {"url": "https://www.google.com/search?q=", "str": "G"},
        ...
        self.search_engine_button.setPopupMode(QToolButton.InstantPopup)
        self.search_engine_menu = QMenu(self)
        self.search_engine_button.setMenu(self.search_engine_menu)
        ...
        self.security_report_dialog = SecurityReportDialog(self)
    ...
    def _handle_url_change(self, tab, qurl):
        ...
        if tab == self.tabs.currentWidget():
            print("DEBUG: This is the current tab. Updating URL bar and UI.")
            self.url_bar.setText(qurl.toString())
        else:
            print(f"DEBUG: This is a background tab. Not updating UI.")
    ...
    def create_homepage(self):
        homepage_widget = HomePage(self, is_incognito=True)
    ...
    def update_ui_on_tab_change(self):
        current_tab = self.tabs.currentWidget()
        print(f"DEBUG: Tab change detected. New tab is index {self.tabs.indexOf(current_tab)}")
        if isinstance(current_tab, BrowserTab):
            self.url_bar.setText(current_tab.browser.url().toString())
            self.update_security_icon(current_tab)
        else:
            self.url_bar.clear()
    ...
    def show_history_dialog(self):
        QMessageBox.information(self, "Incognito Mode", "History is not saved or viewable in Incognito Mode.")
    ...
    def update_search_engine_menu(self):
        ...
        for name, data in self.search_engines.items():
            action = QAction(data['str'] + " - " + name, self)
            action.triggered.connect(lambda checked, n=name: self.set_search_engine(n))
            self.search_engine_menu.addAction(action)

    def update_search_engine_button_icon(self):
        self.search_engine_button.setToolButtonStyle(Qt.ToolButtonTextOnly)
    ...
    def handle_ssl_errors(self, tab, ssl_errors):
        ...
        self.ssl_errors_list = list(set(error_descriptions))
        tab.security_status = {"status": "Connection Error", "description": "This site has a certificate issue. " + " ".join(self.ssl_errors_list), "icon_path": os.path.join(ICON_PATH, "Unsafe_Icon.svg"), "icon_color": "#ffc107"}
        self.update_security_icon(tab)
        
    def handle_request_blocked(self, tab):
        print("DEBUG: Request blocked detected.")
        self.blocked_requests.append("A resource on this page was blocked for security reasons.")
        tab.security_status = {"status": "Blocked Content", "description": "Content on this page was blocked due to a security violation.", "icon_path": os.path.join(ICON_PATH, "Blocked_Icon.svg"), "icon_color": "#dc3545"}
        self.update_security_icon(tab)
    ...
    def show_security_report(self):
        ...
        status = current_tab.security_status.get("status", "Unknown Status")
        description = current_tab.security_status.get("description", "No information available.")
        color = current_tab.security_status.get("icon_color", "#000000")
        self.security_report_dialog.show_report(status, description, color)
...
class MyMainWindow(QMainWindow):
    def __init__(self):
        ...
        incognito_btn.setIcon(QIcon(os.path.join(ICON_PATH, "Incognito_Icon.svg")))
        incognito_btn.setIconSize(QSize(24, 24))
        incognito_btn.clicked.connect(self.open_incognito_window)
        sidebar.addWidget(incognito_btn)
    ...
    def open_incognito_window(self):
        incognito_window = IncognitoWindow()
        incognito_window.show()
        global incognito_windows
        incognito_windows.append(incognito_window)
        incognito_window.destroyed.connect(lambda: incognito_windows.remove(incognito_window))
    ...
    def update_security_icon(self, tab):
        print(f"DEBUG: Attempting to update icon for tab at index {self.tabs.indexOf(tab)}. Current tab is index {self.tabs.indexOf(self.tabs.currentWidget())}.")
        if isinstance(tab, BrowserTab):
            icon_path = tab.security_status.get("icon_path")
            if icon_path and os.path.exists(icon_path):
                self.security_icon_button.setIcon(QIcon(icon_path))
